...
🎯 **目的**  
動体検出で侵入を検知し、即座に警告・通知・記録を行う防犯監視システム

## 🧩 機能仕様

| 機能 | 内容 |
|------|------|
| 人物検知 | PIR人感センサ（SB412A）による侵入検知 |
| 警告出力 | 検知後、電子ブザーを10秒間鳴動 |
| LED点滅 | 2個のLEDを0.5秒周期で交互に点滅（逆位相）、10秒間継続 |
| LINE通知 | 検知時にLINE Messaging APIでメッセージを送信 |
| 映像記録 | 検知後の30秒間の映像をSDカードに保存（MJPEG形式） |
| ストリーミング | リアルタイムMJPEGストリーミング配信 |
| Webインターフェース | ライブプレビュー、システム状態表示 |
...
| 人感センサ SB412A | GPIO D0 (GPIO 1) | 出力信号で検知 |
...
- **動体検出**: PIR人感センサ（SB412A）のデジタル出力を監視
- **SD_MMC**: SDカードへのMJPEG動画保存
- **HTTPClient**: LINE Messaging API送信
- **ESPAsyncWebServer**: ストリーミングURL提供、Webインターフェース
...
1. **設定確認**: `http://[IPアドレス]/line-config` にアクセス
2. **手動テスト**: `http://[IPアドレス]/test-line` でテスト送信
3. **ログ確認**: シリアルモニターで詳細なデバッグ情報を確認

詳細は [docs/LINE_DEBUG_GUIDE.md](docs/LINE_DEBUG_GUIDE.md) を参照
